...
class NetworkManager {
    ...
    {
        do {
            let result: SessionRefreshOutput = try await self.fetch(endpoint: "com.atproto.session.refresh",httpMethod: .POST,  authorization: self.user.refreshJwt, params: Optional<Bool>.none, retry: false)
    ...
    func fetch<T: Decodable, U: Encodable>(endpoint: String, httpMethod: httpMethod = .GET, authorization: String? = nil, params: U? = nil, retry: Bool = true) async throws -> T {
                ...
                if authorization != nil && retry == true {
                    if xrpcerror.error == "ExpiredToken" {
                        if await self.refreshSession() {
                            return try await self.fetch(endpoint: endpoint, httpMethod: httpMethod, authorization: self.user.accessJwt, params: params, retry: false)
                        }
                    }
